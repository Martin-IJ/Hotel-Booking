  ...
  return (
    <div className="container mx-auto">
      <div className="grid md:grid-cols-2 relative gap-5 px-3">
        <div className="h-[410px] lg:h-[540px] relative rounded-2xl overflow-hidden">
        ...
        <div className="hidden md:grid grid-cols-2 h-full gap-5">
          {displayPhotos.map((photo, index) => (
            <div
              key={index}
              className="h-48 lg:h-64 rounded-2xl overflow-hidden"
            >
              <Image
                height={150}
                width={150}
          ...
          {remainingPhotosCount > 0 && (
            <div
              className="cursor-pointer relative h-48 lg:h-64 rounded-2xl overflow-hidden"
        ...
        {showModal && (
          <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-90 z-[55]">
            <div className="h-[400px] md:h-[75vh] w-[85%] md:w-[700px] relative shadow-2xl shadow-black rounded-lg">
              <Image
                src={photos[currentPhotoIndex].url}
                alt={`Room Photo ${currentPhotoIndex + 1}`}
                width={150}
              ...
              <div className="flex justify-between items-center py-3">
                <button className="flex space-x-2 items-center text-white">
                  <FaArrowLeft
              ...
              <button
                className="absolute top-2 right-2 text-white text-lg drop-shadow-md"
                onClick={closeModal}
              >
                <MdCancel className="font-medium text-3xl text-tertiary-dark" />
              </button>
            </div>
          </div>
        )}
